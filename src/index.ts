...
function websocketToObservable(url: string) {
  const ws = new WebSocket(url)

  new AnonymousSubject<BufferLike>(
    {
      next: (x) => {
      ...
      complete: () => {
		...
		new Observable((subj) => {
			ws.on('message', (msg) => subj.next(msg))
			ws.on('error', (err) => subj.error(err))
			ws.on('close', () => subj.complete())
...
class HomeAssistantWebsocket {
  ...
  public subscribeToEvents(eventType?: string): Observable<HAEvent> {
    ...
    d(
      ...
      eventType ? `of type: ${eventType}` : 'all events'
    )
